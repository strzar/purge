...
positive = f"Positive_{model_name}.json"
negative = f"Negative_{model_name}.json"
pair = f"Pair_{model_name}.json"
...
def main(args):
    ...
    for subject, intros in dataset.items():
        ...
        with open(f'Pairs_{model_name}.json', 'a') as f:
            json.dump(data, f)
            f.write('\n')

    with open(f'Pairs_{model_name}.json', 'r') as file:
        cnt = 0
        for line in file:
            cnt += 1
            output_positive = []
