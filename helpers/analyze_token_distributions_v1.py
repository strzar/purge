...
def compute_forbidden_sequence_mass(
    ...
    of each forbidden phrase appearing and aggregate across prompts.

    Returns:
      {
        "per_phrase": { phrase: mass },
        "total": total_mass_over_all_phrases,
        "avg_probs": np.ndarray[vocab_size]   # <<< NEW: average token probs
      }
    """
    model.eval()
    ...
    prob_sums = torch.zeros(vocab_size, dtype=torch.float64, device="cpu")  # <<< NEW
    total_steps = 0  # <<< NEW
    ...
    for ex in tqdm(dataset, desc="Running generation & accumulating phrase mass"):
        ...
        while generated < max_new_tokens:
            ...
            prob_sums += probs_cpu.to(dtype=torch.float64)  # <<< NEW
            total_steps += 1                                # <<< NEW

            # Choose the next token (greedy; you can change to sampling)
    ...
    avg_probs = (prob_sums / max(total_steps, 1)).numpy()  # <<< NEW

    return {
        "per_phrase": agg_phrase_mass,
        "total": total_mass,
        "avg_probs": avg_probs,  # <<< NEW
    }
...
def main():
    ...
    # Pick top-K tokens by absolute difference
    vocab_size = len(base_avg)
    k = min(TOP_K_PLOT, vocab_size)
    top_indices = np.argsort(abs_diff)[::-1][:k]

    # Decode tokens for labels (escaping newlines/spaces makes plot more readable)
    labels = []
    ...
    print(f"({results['relative_reduction']*100:.2f}% drop)")
