...
only the specified fields (if dict-based JSON) or directly over list-of-strings JSON.

Supports:
- JSON as list of dicts with fields
- JSON as dict
- JSON as list of plain strings
...
# ---- Field extraction ----
def extract_records(data: Any, fields: Optional[List[str]] = None) -> List[str]:
    """
    Extract records from JSON data.
    - If list of strings -> return directly
    - If list of dicts -> collect specified fields
    - If dict -> collect specified fields
    """
    out: List[str] = []
    if isinstance(data, list):
        if all(isinstance(x, str) for x in data):
            # Case: list of plain strings
            out.extend(data)
        else:
            for rec in data:
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Count Phi-3 Mini 4K tokens for specified fields in JSON "
                    "across first N numbered dirs."
    )
    ap.add_argument("--root", type=Path, required=True,
                    help="Root folder containing numbered subdirectories.")
    ap.add_argument("--tokenizer-json", type=Path, required=True,
                    help="Path to Phi-3 tokenizer.json.")
    ap.add_argument("--json-file", type=str, required=True,
                    help="Filename inside each dir (e.g., reject_phi.json).")
    ap.add_argument("--fields", nargs="*",
                    help="Field names to extract from JSON dicts "
                         "(ignored for list-of-strings JSON).")
    ap.add_argument("--limit", type=int, default=21,
                    help="How many numeric dirs to scan (default: 21)")
    ap.add_argument("--csv-out", type=Path, default=Path("field_token_stats.csv"),
    ...
    # Console report
    field_info = f"fields {args.fields}" if args.fields else "list-of-strings"
    print(f"Phi-3 Mini 4K token stats over {field_info}:")
    for r in rows:
    ...
    if rows:
        ...
        print(f"OVERALL: scanned {len(rows)} dirs, "
              f"total_records={overall_records}, total_tokens={overall_tot}, "
              f"avg_tokens={overall_avg:.2f}")

    # Write CSV
    with args.csv_out.open("w", newline="", encoding="utf-8") as csvfile:
        writer = csv.DictWriter(csvfile,
                                fieldnames=["dir","file","records_counted","total_tokens","avg_tokens"])
        writer.writeheader()
